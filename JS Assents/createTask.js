...
function isVitalTask(task) {
    const vitalPriority = task.priority === "Extreme" || task.priority === "Moderate";
    const vitalStatus = task.status === "Not Started" || task.status === "In Progress";
...
function renderDashboardLists() {
    ...
    taskListGroup.forEach(task => {
        ...
        if (task.status === "Completed") {
            if (compeltedList) compeltedList.insertAdjacentHTML("beforeend", cardHTML);
        } else {
            nonCompletedCount++;
...
function setActiveVitalTask(taskIdToActivate) {
    ...
    if (vitalTaskList) {
        ...
        items.forEach(item => {
            ...
            if (itemId === taskIdToActivate) item.classList.add("active");
        });
    }
    const task = taskListGroup.find(t => t.id === taskIdToActivate);
    if (task) renderVitalTaskDetails(task);
}
function renderVitalList() {
    ...
    items.forEach(item => {
        ...
        item.addEventListener("click", () => setActiveVitalTask(id));
    });
    if (vitalTasks.length > 0) setActiveVitalTask(vitalTasks[0].id);
    else if (vitalTaskDetails) vitalTaskDetails.innerHTML = "";
}
...
function setActiveMyTask(taskIdToActivate) {
    ...
    const task = taskListGroup.find(t => t.id === taskIdToActivate);
    if (task) renderMyTaskDetails(task);
}
function renderMyTaskList() {
    ...
    items.forEach(item => {
        const id = Number(item.getAttribute("data-task-id"));
        item.addEventListener("click", () => setActiveMyTask(id));
    });
    if (taskListGroup.length > 0) setActiveMyTask(taskListGroup[0].id);
    else if (myTaskDetails) myTaskDetails.innerHTML = "";
}
function handleDeleteTask(taskIdToDelete) {
    ...
    if (removedTask.status === "Completed") completedCount = Math.max(0, completedCount - 1);
    else if (removedTask.status === "In Progress") inProgressCount = Math.max(0, inProgressCount - 1);
...
function openEditTaskModal(task) {
    ...
    allPriorityCbs.forEach(cb => {
        cb.checked = false;
        if (cb.parentElement.textContent.trim() === task.priority) cb.checked = true;
    });
    const allStatusCbs = document.querySelectorAll('.taskStatus ul li input[type="checkbox"]');
    allStatusCbs.forEach(cb => {
        cb.checked = false;
        if (cb.parentElement.textContent.trim() === task.status) cb.checked = true;
    });
    if (task.customFields) {
        ...
        dynamicDivs.forEach(div => {
            ...
            if (val) {
                ...
                cbs.forEach(cb => {
                    cb.checked = false;
                    if (cb.parentElement.textContent.trim() === val) cb.checked = true;
                });
            }
        });
    }
    if (task.imgSrc) {
        imgItems.forEach(span => {
            ...
            if (img && img.getAttribute("src") === task.imgSrc) span.classList.add("selected");
        });
    }
    const menu = document.getElementById("taskMenu");
    if (menu) menu.style.top = "0";
...
if (createTaskBtn) {
    createTaskBtn.onclick = function () {
        ...
        if (descErr) descErr.textContent = "";
        let valid = true;
        if (!titleValue) { if (titleErr) titleErr.textContent = "Enter title"; valid = false; }
        if (!dateValue) { if (dateErr) dateErr.textContent = "Enter date"; valid = false; }
        if (!descValue) { if (descErr) descErr.textContent = "Enter Description"; valid = false; }
        if (!valid) return;
