(function () {
  ...
  document.addEventListener("click", function (e) {
    ...
    if (addCategoryItemBtn) {
      ...
      openModal("Add " + title, title + " Name", "Create", "", "add-category-item", { tbody, title });
      return;
    }
  });

  if (categBack) {
    categBack.addEventListener("click", function (e) {
      stopLink(e);
      showHero();
    });
  }
  ...
  if (editorBack) {
    editorBack.addEventListener("click", function (e) {
      stopLink(e);
      showHero();
    });
  }
  ...
  if (addCategoryBtn) {
    addCategoryBtn.addEventListener("click", function () {
      openModal("Create Categories", "Category Name", "Create", "", "add-category", null);
    });
  }

  if (addStatusBtn) {
    addStatusBtn.addEventListener("click", function () {
      openModal("Add Task Status", "Task Status Name", "Create", "", "add-status", null);
    });
  }

  if (addPriorityBtn) {
    addPriorityBtn.addEventListener("click", function () {
      openModal("Add Task Priority", "Task Priority Title", "Create", "", "add-priority", null);
    });
  }

  if (statusBody) {
    statusBody.querySelectorAll('button[data-action="edit"]').forEach((b) => {
      b.dataset.rowType = "status";
    });
    rebuildSN(statusBody);
  }

  if (priorityBody) {
    priorityBody.querySelectorAll('button[data-action="edit"]').forEach((b) => {
      b.dataset.rowType = "priority";
    });
    rebuildSN(priorityBody);
  }

  function dispatchUpdate() {
    const event = new CustomEvent("taskCategoriesUpdated");
  ...
  if (editorPrimary) {
    editorPrimary.addEventListener("click", function () {
      ...
      if (activeMode === "add-category") {
        ...
        if (hr) {
          categHero.insertBefore(created.section, hr);
        } else {
          categHero.appendChild(created.section);
        }
        closeModal();
        dispatchUpdate();
        return;
      ...
      if (activeMode === "add-category-item") {
        const tbody = activePayload && activePayload.tbody ? activePayload.tbody : null;
        const title = activePayload && activePayload.title ? activePayload.title : "Item";
        if (!tbody) return;
        addRowToTbody(tbody, value, "category", { categoryId: tbody.dataset.categoryId || "", categoryTitle: title });
        closeModal();
        dispatchUpdate();
        return;
      }

      if (activeMode === "edit-row") {
